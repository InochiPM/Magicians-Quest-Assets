...
for i in range(0, len(DUMP)):
    ...
    header_info_temp = {}
    try:
        readEntry = DUMP[i]["deepl"]
    except:
        readEntry = DUMP[i]["JPN"]
    for c in range(0, len(readEntry)):
        CMDCheck = readEntry[c].count("<CMD>")
        if (CMDCheck > 0): 
            entry.append(ConvertCMDtoDol(readEntry[c]))
        else: entry.append(readEntry[c].replace("<BREAK>", "▼").encode("UTF-16-LE"))
